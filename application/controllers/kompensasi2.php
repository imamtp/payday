...
class kompensasi2 extends MY_Controller {
    ...
    function pajakjantonov($idpelamar,$pphsebulan,$tahun)
    {
        ...
        from payrolldata
        where idpelamar = $idpelamar and (startdate between '$tahun-01-01' and '".date("Y-m-t", strtotime($tahun.'11-01'))."')")->row();
        if($q->total==null)
        {
            return $pphsebulan;
        } else {
            return $q->total+$pphsebulan;
        }
    }
    ...
    function gaji()
    {
        ...
        $BpjsKecelakanCompany = 21600;
        $BpjsKematianCompany = 27000;
        $BpjsTuaCompany = 333000;

        $UpahPengurangPajak = 180000;

        $BpjsTuaEmploy = 180000;
    }

    function tunjanganpajak2()
    {
        $npwp = true;
        $PajakType = 'NETT';
        $BulanMasuk = 1;
        ...
        $totallembur = 0;
        $upahlemburKurangPajak = $totallembur;

        $BpjsKecelakanCompany = 21600;
        $BpjsKematianCompany = 27000;
        $BpjsTuaCompany = 333000;
        $benefitCmp = $BpjsKecelakanCompany + $BpjsKematianCompany + $BpjsTuaCompany;

        $UpahPengurangPajak = 180000;
        $utPengurangPajak = $UpahPengurangPajak;
        ...
        // foreach ($qpeg->result() as $rpeg) {
            ...
            // $data[$i]['masapajaksetahun'] = diffInMonths($data[$i]['tglgajipertama'],$akhirtahun) > 12 ? 12 : diffInMonths($data[$i]['tglgajipertama'],$akhirtahun);
             $data[$i]['masapajaksetahun'] = $MasaPajak;

            $data[$i]['totalUT'] = $GajiPokok+$TunJabatan+$TunOper+$TunMakan+$BpjsKecelakanCompany+$BpjsKematianCompany;
            $penghasilanbrutoParams = $GajiPokok+$TunJabatan+$TunOper+$TunMakan+$BpjsKecelakanCompany+$BpjsKematianCompany;
            $data[$i]['totalUTT'] = 0;


            $tunjanganpajak=0;
            $penghasilanbrutoOld = $penghasilanbrutoParams;
                    for($j=0;$j<$MasaPajak;$tunjanganpajak++) {
                       // echo $tunjanganpajak.' ';
                        ...
                        $pphsetsebulan = ceil($data[$i]['pphsettahun']/$data[$i]['masapajaksetahun']);
...
// echo $data[$i]['masapajaksetahun']." ".diffInMonths($qtglmasuk->tglmasuk,$obj->tglakhirjabatan);
    ...
    //                 exit;
            ...
            // echo '</pre>';
                        if($data[$i]['masapajaksetahun']<12 && diffInMonths($qtglmasuk->tglmasuk,$obj->tglakhirjabatan)<12)
                        {
                            $data[$i]['takehomepay'] = $this->penggajian2($idcompany,$startdate,$enddate,$obj->idpelamar,$tunjanganpajakTMP,$data[$i]['masapajaksetahun']);
                            $obj->takehomepay = $data[$i]['takehomepay'];
                        }
