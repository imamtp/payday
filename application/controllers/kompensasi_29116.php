...
class kompensasi extends MY_Controller {
    ...
    function pphteratur($pkpsetahunteratur,$data,$i,$masapajaksetahun)
    {
        ...
        ///////////// pph5%tahun
                        ...
                        $pphsetsebulan = round($data[$i]['pphsettahun']/$masapajaksetahun);
...
//echo $rpeg->namalengkap.' '; 
        ...
        // exit;
            ...
            $obj->bulanakhirjabatan = $tglakhirjabatanArr[1];
            ...
            $obj->totalUT = $totalUT;
            // echo 'utPengurangPajak'.$utPengurangPajak;
           ...
           // echo $this->db->last_query().'<br>';
            ...
            // echo ' utTPengurangPajak'.$utTPengurangPajak;
...
//            exit;
            ...
            $penghasilanTT+=$obj->totalUTT;
// echo ' utTTahunPengurangPajak'.$utTTahunPengurangPajak;
            ///////////////////////////////////////////////////////////////////////////////////////////////
            ...
            // echo ' upahlemburKurangPajak'.$data[$i]['upahlemburKurangPajak'];
...
//                                        }
            ...
            // echo ' benefitPengurangPajak'.$benefitPengurangPajak;
...
// echo $data[$i]['masapajaksetahun']." ".diffInMonths($qtglmasuk->tglmasuk,$obj->tglakhirjabatan);
                    ...
                    $data[$i]['tunjanganpajakBuatPPH'] = $tunjanganpajak;
                    ...
                    $data[$i]['tunjanganpajakBuatPPH'] = ceil($tunjanganpajak[0]);
                }
            ...
            $obj->penerimaanbrutoteratur =  $data[$i]['penerimaanbruto']-$penghasilanTT;
            $obj->penghasilanTT = $penghasilanTT;
            ...
            $biayajabatan = ceil($data[$i]['penerimaanbruto']*0.05);
                        ...
                        // exit;
                        $totalUpahPengurangPajak = ($utPengurangPajak+$utTPengurangPajak+$data[$i]['upahlemburKurangPajak']+$benefitPengurangPajak);

                        // echo $totalUpahPengurangPajak;
                        $penghasilannet = $data[$i]['penerimaanbruto']-($benefitEmp+$data[$i]['biayajabatan'])-$totalUpahPengurangPajak;
...
//                        $data[$i]['netosetahun'] = ($penghasilannet*$data[$i]['masapajaksetahun'])+$penghasilanTT;
                        ...
                        // $pphsetsebulan = $data[$i]['pphsettahun']/$data[$i]['masapajaksetahun'];
                        $pphsetsebulan = round($data[$i]['pphsettahun']/$obj->masapajaksetahun);
...
//echo $obj->pphsettahun.'-'.$obj->pphsettahunteratur;
                        ...
                        if($data[$i]['masapajaksetahun']<12)
                        {
                            ...
                            {
                                $obj->pphterminasi = round($obj->pphsettahun/$data[$i]['masapajaksetahun']);
                            } else {
                                $obj->pphterminasi = 0;
                            }
...
// echo $data[$i]['pphsebulan'];
            ...
            if(intval($enddateArr[1]) == 12)
            {
             ...
             //get data sebelumnya = 
                ...
                {
                    $Prev_penerimaanbrutoteratur = 0;
                    $Prev_upahtaktetapakumulasi = 0;
                    $Prev_tunjpajakbenefittaktetapakumulasi = 0;
                } else {
                    if(intval($startdateArr[1])<=10)
                    {
                        $m = '0'.(intval($startdateArr[1])-1);
                    } else {
                        $m = $startdateArr[1];
                    }
                    
                    $qqprev = $this->db->get_where('payrolldata',array('idpelamar'=>$data[$i]['idpelamar'],'bulan'=>$m));
                    if($qqprev->num_rows()>0)
                    {

                        $qprev = $qqprev->row();
                        $Prev_penerimaanbrutoteratur = $qprev->penerimaanbrutoteratur == null ? $obj->penerimaanbrutoteratur : $qprev->penerimaanbrutoteratur;
                        $Prev_upahtaktetapakumulasi = $qprev->upahtaktetapakumulasi == null ? $obj->penghasilanTT : $qprev->upahtaktetapakumulasi;
                        $Prev_tunjpajakbenefittaktetapakumulasi = $qprev->tunjpajakbenefittaktetapakumulasi == null ? $obj->tunjanganpajak : $qprev->tunjpajakbenefittaktetapakumulasi;
                    ...
                    $penerimaanbrutoteratur = $Prev_penerimaanbrutoteratur + ($obj->penerimaanbrutoteratur-$data[$i]['tunjanganpajak']);
                    $upahtaktetapakumulasi = $Prev_upahtaktetapakumulasi + $obj->penghasilanTT;
                    $tunjpajakbenefittaktetapakumulasi = $Prev_tunjpajakbenefittaktetapakumulasi + $data[$i]['tunjanganpajak'];

                }

                // echo '('.$obj->penerimaanbrutoteratur.'-X'.$obj->tunjanganpajak.') * ('.intval($obj->bulanakhirjabatan).'-'.intval($startdateArr[1]).') + '.$penerimaanbrutoteratur.' + '.$upahtaktetapakumulasi.' + ('.$data[$i]['tunjanganpajak'].'*('.$obj->bulanakhirjabatan.'-'.intval($startdateArr[1]).'))+'.$tunjpajakbenefittaktetapakumulasi;
                 $obj->brutopertahun = ($obj->penerimaanbrutoteratur-$obj->tunjanganpajak) * (intval($obj->bulanakhirjabatan)-intval($startdateArr[1])) + $penerimaanbrutoteratur + $upahtaktetapakumulasi + ($data[$i]['tunjanganpajak']*($obj->bulanakhirjabatan-intval($startdateArr[1])))+$tunjpajakbenefittaktetapakumulasi;
                 // echo '<br>'.number_format($obj->brutopertahun);
                 //pengurangBijab
               
                  $v1 = ($obj->brutopertahun*(5/100))/$data[$i]['masapajaksetahun'];
                  if($v1>500000)
                  {
                    $pengurangSetahunBijab = ceil(500000*$data[$i]['masapajaksetahun']);
                  } else {
                    $pengurangSetahunBijab = ceil($obj->brutopertahun*(5/100));
                  }
                // echo $data[$i]['upahlemburKurangPajak'];
                  // =(AV12*(J12-Q12))+BR12
                    //upah pengurang pajak
                $pengurangSetahunKurangPajak = $totalUpahPengurangPajak*(intval($obj->bulanakhirjabatan)-intval($startdateArr[1]))+$this->benefitEmpTahun($data[$i]['idpelamar'],$startdateArr[0],$data[$i]['benefitEmp']);
                // echo $pengurangSetahunKurangPajak;
                // PH KENA PAJAK  =IF(ROUND(BT12-BU12-BV12-BW12,-3)<0,0,ROUND(BT12-BU12-BV12-BW12,-3))
                // echo $obj->brutopertahun.'-'.$pengurangSetahunBijab.'-'.$pengurangSetahunKurangPajak.'-'.$data[$i]['nilaiptkp'];
                $phkenapajak = round($obj->brutopertahun-$pengurangSetahunBijab-$pengurangSetahunKurangPajak-$data[$i]['nilaiptkp'],-3);
                // echo $phkenapajak;
                $data[$i]['pphsettahun'] =  $this->pphsetahun($phkenapajak,$data[$i]['punyanpwp']);
                // $data[$i]['pphsebulan'] = round($data[$i]['pphsettahun']/$data[$i]['masapajaksetahun']);
                // $obj->pphsebulan = $data[$i]['pphsebulan'];
                $obj->pphsebulan = $data[$i]['tunjanganpajakBuatPPH'];
                $data[$i]['pphsebulan'] = $obj->pphsebulan;
                $obj->pphsettahun = $data[$i]['pphsettahun'];

                $obj->pajakjantonov = $this->pajakjantonov($data[$i]['idpelamar'],$obj->pphsebulan,$enddateArr[0]);
                $obj->pajakterbayar = $obj->pajakjantonov-$obj->pphterminasi;
                $obj->pajakterutangdes = $obj->pphsettahun-$obj->pajakterbayar;
                $obj->pajaktotalbayarsetahun = $obj->pajakterbayar+$obj->pajakterutangdes;
...
//                         echo 'biayajabatan'.$rpeg->biayajabatan;
        ...
        if($masapajaksetahun<12)
        {
            ...
            if(intval($startdateArr[0]) == intval($getTglTerminasiArr[0]))
            {
                $objx->pphterminasi = $objx->pphsettahun/$masapajaksetahun;
            } else {
                $objx->pphterminasi = 0;
            }
...
//    $data = explode("text/html", $curldata ); 
  //  $temp = explode("\r\n", $data[1]) ; 
   ...
   function benefitEmpTahun($idpelamar,$year,$benefitnow)
   {
        $q = $this->db->query("select sum(benefitemp) as total
            from payrolldata
            where tahun = '$year' and idpelamar=$idpelamar")->row();
        return $q->total==null ? 0 : $q->total+$benefitnow;
   }

   function pphsetahun($pkpsetahun,$npwp=1)
   {
        $i=0;
        $obj = new stdClass();

        $data[$i]['punyanpwp'] = $npwp;
   ...
   function tesadmon()
   {
    // echo 'asdasd';
           ...
           for($i=1;$i<=$numMonth;$i++)
           {
                ...
                $sql = "select memberTransactionID
                        from coop_member_transaction
                        where memberSavingID=1 and (memberTransactionDate between '".$dateArr[0]."-".$dateArr[1]."-01' and '".$dateArr[0]."-".$dateArr[1]."-".cal_days_in_month(CAL_GREGORIAN, $dateArr[1], $dateArr[0])."')";
                        echo $sql.'<br>';
           }
   }
