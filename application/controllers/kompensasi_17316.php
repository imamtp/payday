            ...
            if ($oke) {
                ...
                while (isset($val[$start])) {
                    ...
                    if($d['0']!='')
                    {
                         ...
                         $data = array(
                                "idpelamar" => $qnik->idpelamar,
                                "startdate" => $sd[2].'-'.$sd[1].'-'.$sd[0],
                                "enddate" => $nd[2].'-'.$nd[1].'-'.$nd[0],
                                "jenisupah" => $d[2],
                                "masukpajak" => $d[5],
                                "fungsipajak" => $d[6],
                                "nominal" => str_replace('.', '', $d[7]),
                                "datein" => $this->tanggalWaktu(),
                                "userin" => $this->session->userdata('username')
                            );
   ...
   function validasiImportUTT($d,$update=false)
   {
        ...
        if($d['1']=='')
        {
            ...
            $qemp = $this->db->query("select nik
                                    ...
                                    WHERE TRUE AND a.display is null and ('".$tgl."' between aa.tglmasuk and aa.tglberakhir OR '".$tgl."' between aaa.tglmasuk and aaa.tglberakhir)  AND (a.idcompany=57) 
