...
class m_datapekerjaanaktif extends CI_Model {
    ...
    function whereQuery() {
        ...
        } else if($this->input->post('tglkeluar1')!=null && $this->input->post('tglkeluar2')==null)
        {
         ...
         if($aktif=='true')
         {
            return "a.display is null and '$datenow' between tglmasuk and tglberakhir $wer";
         } else {
            // $wer = str_replace("WHERE TRUE AND", "WHERE TRUE", $wer);
            // return "$wer";
             return "a.display is null $wer and ('".gmdate('Y-m-d')."' between aa.tglmasuk and aa.tglberakhir)";
        }
    }
